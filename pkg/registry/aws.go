	...
	preflight := func() error {
		...
		preflightOnce.Do(func() {
			...
			if err != nil {
				...
				logger.Error("fetching region for AWS", zap.Error(err))
				return
			}
		...
		for name, creds := range imageCreds {
			...
			if strings.HasSuffix(domain, ecrHostSuffix) {
				...
				if len(bits) != 6 || bits[1] != "dkr" || bits[2] != "ecr" {
					logger.Warn("AWS registry domain not in expected format <account-id>.dkr.ecr.<region>.amazonaws.com", zap.String("domain", domain))
					continue
				}
				accountID := bits[0]
				region := bits[3]
				...
				if preflightErr != nil {
					logger.Warn(
						"AWS auth implied by ECR image, but AWS API is not available. You can ignore this if you are providing credentials some other way (e.g., through imagePullSecrets)",
						zap.String("image", name.String()),
				...
				if okToUseAWS {
					if err := ensureCreds(domain, region, accountID, time.Now()); err != nil {
						logger.Warn(
							"unable to ensure credentials for ECR",
